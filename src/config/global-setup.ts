...
async function globalSetup(config: FullConfig) {
  console.log('🚀 Starting global setup for Playwright + Cucumber Framework...');
  console.log('══════════════════════════════════════════════════════════════');
...
async function setupAuthentication(_config: FullConfig): Promise<void> {
  ...
  const needsAuth = process.env['SETUP_AUTH'] === 'true' || 
                   process.env['BASE_URL']?.includes('staging') ||
                   process.env['BASE_URL']?.includes('production');
  
  if (!needsAuth) {
    console.log('   ℹ️  Authentication setup skipped (not required for demo environment)');
    return;
  }
  ...
  try {
    ...
    try {
      await page.waitForSelector('[data-testid="username"], input[name="username"]', { timeout: 5000 });
      ...
      await page.fill('[data-testid="username"], input[name="username"]', username);
      await page.fill('[data-testid="password"], input[name="password"]', password);
      ...
      const storageStatePath = path.join('storage-states', 'authenticated.json');
...
async function initializeTestEnvironment(): Promise<void> {
  ...
  const testDataFiles = [
    {
      ...
      content: {
        ...
        demo: { username: 'demo@playwright.dev', password: 'demo123', role: 'demo' }
      }
    },
    {
  ...
  process.env['PLAYWRIGHT_TEST_BASE_URL'] = process.env['BASE_URL'] || 'https://demo.playwright.dev';
...
async function setupMCPIntegration(): Promise<void> {
  ...
  if (fs.existsSync(mcpConfigPath)) {
    try {
      ...
      console.warn('   ⚠️  Failed to load MCP configuration:', (error as Error).message);
    }
  } else {
...
function logEnvironmentInfo(): void {
  ...
  console.log(`   Base URL: ${process.env['BASE_URL'] || 'https://demo.playwright.dev'}`);
