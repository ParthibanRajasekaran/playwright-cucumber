...
async function cleanupAuthenticationStates(): Promise<void> {
  ...
  try {
    ...
    if (cleanedCount === 0) {
      ...
      console.log(
        `   ✓ Cleaned up ${cleanedCount} old authentication state(s)`,
      );
    }
  } catch (error) {
    console.warn(
      "   ⚠️  Failed to clean authentication states:",
...
async function cleanupTemporaryFiles(): Promise<void> {
  ...
  const tempPaths = ["temp", ".temp", "test-tmp", "playwright-temp"];
...
async function archiveTestArtifacts(): Promise<void> {
  ...
  try {
    ...
    if (archivedCount > 0) {
      console.log(
        `   ✓ Archived ${archivedCount} artifact director(ies) to ${archivePath}`,
      );
...
async function cleanupMCPIntegration(): Promise<void> {
  ...
  console.log(
    "   ⚠️  MCP cleanup is currently a placeholder for future implementation",
  );
}
...
function generateFinalSummary(): void {
  ...
  try {
    ...
    console.warn(
      "   ⚠️  Failed to save execution summary:",
      (error as Error).message,
    );
  }
  ...
  console.log(
    `   Duration: Test session completed at ${summaryData.teardownTimestamp}`,
  );
  console.log(
    `   Environment: ${summaryData.environment.baseUrl} (${summaryData.environment.browser})`,
  );
  console.log(
    `   Artifacts: ${Object.values(summaryData.artifactsGenerated).filter(Boolean).length} types generated`,
  );
  console.log(
    `   Platform: ${summaryData.platform} (Node.js ${summaryData.nodeVersion})`,
  );
