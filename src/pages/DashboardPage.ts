...
export class DashboardPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.userMenu = page.locator('[data-testid="user-menu"], .user-menu, .dropdown-toggle').filter({ visible: true });
    this.logoutButton = page.locator('a[href*="logout"].button.secondary.radius');
    this.navigationMenu = page.locator('[data-testid="nav-menu"], .navigation, .main-nav');
    this.welcomeMessage = page.locator('p').filter({ hasText: /welcome|login/i });
    this.userProfile = page.locator('[data-testid="user-profile"], .user-profile, .profile-link');
  ...
  async hasNotifications(): Promise<boolean> {
    if (await this.isVisible(this.notificationBell)) {
      const notificationCount = await this.notificationBell.locator('.notification-count, .badge').textContent();
  ...
  async navigateToMenuItem(menuItem: string): Promise<void> {
    const menuLink = this.navigationMenu.locator('a, button').filter({ hasText: new RegExp(menuItem, 'i') });
  ...
  async isSidebarExpanded(): Promise<boolean> {
    if (await this.isVisible(this.sidebar)) {
      ...
      return !sidebarClass.includes('collapsed') && !sidebarClass.includes('minimized');
    }
    return false;
  }

  async toggleSidebar(): Promise<void> {
    const toggleButton = this.page.locator('[data-testid="sidebar-toggle"], .sidebar-toggle, .menu-toggle').first();
  ...
  async verifySuccessfulLogin(): Promise<boolean> {
    try {
      ...
      const urlCheck = currentUrl.includes('/secure') || currentUrl.includes('/dashboard') || currentUrl.includes('/home');
